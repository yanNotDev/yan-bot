...
class help(commands.Cog):
    ...
    async def help(self, ctx, cmd=None):
        ...
        if cmd is None:
            embed.add_field(name="Skyblock", value="`rates`, `stats`, `cata`", inline=False)
            ...
            embed=Embed(title="Rates", description="Calculate coins per hour from farming.", colour=ctx.guild.me.color)
    ...
    async def info(self, ctx):
        embed=Embed(title="yan", description="A skyblock bot. Since I'm bad, you can expect a lot of bugs (please report them).", colour=ctx.guild.me.color)
